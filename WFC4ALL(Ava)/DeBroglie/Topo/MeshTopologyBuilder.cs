...
public class MeshTopologyBuilder {
    ...
    /// <summary>
    ///     Registers face1 and face2 as adjacent, moving in direction from face1 to face2 and inverseDirection from face2 to
    ///     face1.
    /// </summary>
    public void Add(int face1, int face2, Direction direction, Direction inverseDirection) {
        ...
        if (neighbours.GetLength(0) <= maxFace) {
            GraphTopology.NeighbourDetails[,] newNeighbours
                = new GraphTopology.NeighbourDetails[maxFace + 1, directions.Count];
    ...
    public GraphTopology GetTopology() {
        if (pendingInverses.Count > 0) {
            ...
            throw new Exception(
                $"Some face adjacencies have only been added in one direction, e.g. {kv.Key.Item1} -> {kv.Key.Item2}");
        }
